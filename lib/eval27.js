...
/**
 * `handIndex` for five loose cards, without allocating.
 *
 * The equity loop indexes tens of millions of hands, and sorting a fresh copy
 * of five cards every time is most of the cost. This sorts in place with a
 * nine-comparison network over one reused buffer, so it is not reentrant - fine
 * for a single-threaded inner loop and nothing else.
 */
const scratch5 = new Int32Array(5);

function ascend(i, j) {
  if (scratch5[i] > scratch5[j]) {
    const swap = scratch5[i];
    scratch5[i] = scratch5[j];
    scratch5[j] = swap;
  }
}

export function index5(a, b, c, d, e) {
  scratch5[0] = a;
  scratch5[1] = b;
  scratch5[2] = c;
  scratch5[3] = d;
  scratch5[4] = e;
  ascend(0, 1); ascend(3, 4); ascend(2, 4); ascend(2, 3); ascend(0, 3);
  ascend(0, 2); ascend(1, 4); ascend(1, 3); ascend(1, 2);
  return CHOOSE[scratch5[0]][1] + CHOOSE[scratch5[1]][2] + CHOOSE[scratch5[2]][3]
    + CHOOSE[scratch5[3]][4] + CHOOSE[scratch5[4]][5];
}
