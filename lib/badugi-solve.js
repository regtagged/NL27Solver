...
export class BadugiSolver {
  ...
  traverse(nodeId, traverser) {
    ...
    const actions = node.actions.length;
    const strategy = this.strategyAt(nodeId, hand);
    ...
    if (seat !== traverser) {
      ...
      return this.descend(node, picked, traverser, seat, drawing);
    }

    const values = new Float64Array(actions);
    let value = 0;
    for (let a = 0; a < actions; a += 1) {
      values[a] = this.descend(node, a, traverser, seat, drawing);
  ...
  descend(node, action, traverser, seat, drawing) {
    if (!drawing) return this.traverse(node.actions[action].child, traverser);
    const at = seat * this.rounds + this.roundOf(node);
    const before = this.drawn[at];
    this.drawn[at] = node.actions[action].option;
    const value = this.traverse(node.actions[action].child, traverser);
  ...
  run(iterations, onProgress) {
    for (let i = 0; i < iterations; i += 1) {
      ...
      for (let seat = 0; seat < this.players; seat += 1) {
        this.drawn.fill(-1);
        this.traverse(this.tree.root, seat);
      }
