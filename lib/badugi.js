...
export function score(cards) {
  // Fewer than four cards is a real question, not a malformed one: choosing
  // what to keep means scoring three cards, or two, against each other. The
  // size digit still counts up from four, so a three-card keep and a four-card
  // holding that only makes three compare as the same thing - which is the
  // comparison a keep needs.
  const held = cards.length;
  let best = Infinity;
  for (let mask = 1; mask < (1 << held); mask += 1) {
    let ranks = 0;
    let suits = 0;
    let count = 0;
    let ok = true;
