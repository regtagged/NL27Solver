...
/** Each display row's bucket in the solver's abstraction, worked out once. */
function bucketsForRows(rows, limits = DEFAULT_LIMITS) {
  if (bucketCache && bucketCache.rows === rows) return bucketCache.at;
  const table = buckets(limits).table;
  const at = rows.map((row) => table[handIndex(parseHand(row.cards.join(' ')))]);
  bucketCache = { rows, at };
  return at;
}
 ...
 */
export function strategyTree(solver, nodeId, rows, limits = DEFAULT_LIMITS) {
  const node = solver.nodes[nodeId];
  if (!node || node.kind !== 'decision') return null;

  const at = bucketsForRows(rows, limits);
