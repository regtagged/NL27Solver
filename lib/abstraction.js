 ...
 * - **draw one** - the four lowest distinct ranks, kept.
 * - **draw two** - the three lowest distinct ranks, kept.
 *
 ...
 * the seven cannot make better than an eight-five. The one keep this does not
 * offer is the deliberately looser draw, which is a refinement, not a rule.
 */

import { rankOf, suitOf, RANKS } from './cards.js';
import { score, categoryOf, handIndex, HAND_COUNT, HIGH_CARD } from './eval27.js';
...
export function analyse(cards) {
  ...
  const keep4 = distinct.length >= 4 ? distinct.slice(0, 4) : null;
  const keep3 = distinct.length >= 3 ? distinct.slice(0, 3) : null;
...
export function keepFor(cards, option) {
  ...
  const want = option === DRAW_ONE ? 4 : 3;
  const { distinct } = analyse(cards);
  if (distinct.length < want) return null;

  const ranks = distinct.slice(0, want);
  const choices = ranks.map((rank) => cards.filter((card) => rankOf(card) === rank));
...
export function describe(cards) {
  ...
  if (info.made) return `${show([...info.patRanks].reverse())} pat`;
