 ...
 */
export function distribute(contributions, folded, ranks, dead = 0, into = null) {
  const players = contributions.length;
  // A caller in a hot loop passes its own array rather than making one per pot.
  const payouts = into ?? new Float64Array(players);
  if (into) into.fill(0);
