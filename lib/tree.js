 ...
 * constant.
 */
export function drawActionsFor(config) {
  const most = config.maxDraw ?? 2;
...
function limitActions(state, config) {
  ...
  if (state.bets < (config.limit.cap ?? 4) && behind > toCall) {
...
export function legalActions(state, config) {
  if (state.street === DRAWING) return drawActionsFor(config);
  const seat = state.toAct;
  if (seat < 0) return [];
...
export function buildTree(config = defaultConfig()) {
  ...
  function visit(state) {
    ...
    if (state.street === DRAWING) {
      ...
      const id = add(node);
      seen.set(key, id);
      node.actions = drawActionsFor(config).map((action) => ({
        ...action,
