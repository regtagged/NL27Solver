 ...
 */
export function drawActionsFor(config, round = 0, seat = -1) {
  let ceiling = config.maxDraw ?? 2;
  // `maxDrawBySeat` is keyed on position name, so a rule can say what everybody
  // knows: the big blind defends wide and is the one seat that draws three.
  if (config.maxDrawBySeat && seat >= 0) {
    const name = positionNames(config.players)[seat];
    if (config.maxDrawBySeat[name] !== undefined) ceiling = config.maxDrawBySeat[name];
  }
...
export function positionNames(players) {
  ...
  // Seat 2 is first to act and so is UTG - except three-handed, where it is
  // also the last seat, and a button is what that is. Getting this wrong is
  // quiet and expensive: a rule keyed on seat names, like which seats may cold
  // call, silently stops applying to the seat it was written for.
  if (players > 3) names[2] = 'UTG';
  return names;
}
...
export function tripleDrawConfig(overrides = {}) {
  return {
    ...
    maxToDraw: 3,
    // Which seats may call a raise cold. Null is anyone.
    coldCallSeats: null,
    // Whether a seat not yet in may flat a 3-bet. False makes it four-bet or
    // fold, which is how the pot stays heads-up once it is reraised.
...
/**
 * Whether this seat may cold call - put money in against a raise having put
 * none in of its own accord yet.
 *
 * Cold calling out of position in a limit game is the losing play everybody
 * knows is losing, and it is also the branchiest: every seat that comes in
 * multiplies what follows. Restricting it to the seats that have a reason -
 * the button, which has position on everyone, and the big blind, which is
 * getting a discount to close the action - cuts the tree where the game is
 * already thin. `coldCallSeats` is a list of position names, or null for
 * anyone.
 */
function mayColdCall(state, config) {
  const seat = state.toAct;
  if (state.committed[seat] > blindOf(config, seat)) return true; // already in
  if (state.betLevel <= units(config.bigBlind)) return true; // nothing to cold call

  // Facing a 3-bet, nobody comes in flat: four-bet or fold. The opener is not
  // cold - it has money in already and got past the check above - so this bars
  // the seats that would be entering a raised-and-reraised pot to see draws.
  // `bets` counts the blind as the first, so a 3-bet is the third.
  if (config.coldCallThreeBets === false && state.bets >= 3) return false;

  const seats = config.coldCallSeats;
  if (!seats) return true;
  return seats.includes(positionNames(config.players)[seat]);
}
...
function limitActions(state, config) {
  ...
  // The cap on how multiway a pot gets bars a new seat from coming in at all,
  // by any route. Being barred from *calling* is a narrower thing: a seat told
  // it may not cold call can still raise, which is the whole point of the rule
  // - if you want the pot you have to put it up, not tag along.
  const capped = state.street === PRE_DRAW && !mayEnter(state, config);
  const cold = state.street === PRE_DRAW && !mayColdCall(state, config);
  // A limp is calling the big blind with nobody having raised. Banning it makes
  // an unopened pot raise-or-fold, which is the single biggest prune in the no
  // limit tree and works the same way here.
  const limping = state.street === PRE_DRAW && toCall > 0
    && toCall <= units(config.bigBlind) && state.betLevel <= units(config.bigBlind);

  if (toCall > 0 && !capped && !cold && !(limping && config.allowLimp === false)) {
  ...
  if (!capped && state.bets < (config.limit.cap ?? 4) && behind > toCall) {
...
export function legalActions(state, config) {
  if (state.street === DRAWING) return drawActionsFor(config, state.round, state.toAct);
  const seat = state.toAct;
  if (seat < 0) return [];
...
export function buildTree(config = defaultConfig()) {
  ...
  function visit(state) {
    ...
    if (state.street === DRAWING) {
      ...
      const id = add(node);
      seen.set(key, id);
      node.actions = drawActionsFor(config, state.round, seat).map((action) => ({
        ...action,
