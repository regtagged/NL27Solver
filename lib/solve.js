 ...
 * t-th step by t^γ.
 *
 * **β is 1 here, not the paper's 0, and that is a deliberate departure.** β=0
 * halves negative regret at every step, which is how a wrongly buried action
 * recovers quickly - and also how a dominated one keeps coming back. A sampled
 * iteration's regret is noisy against an edge of a third of a big blind, so an
 * action only stays folded while its debt outweighs the noise, and halving that
 * debt every step caps how far it can outweigh anything. Measured six-handed at
 * the button, hands that draw three cards or more - which should never be
 * opened - were played:
 *
 *   iterations      CFR+     DCFR β=0    DCFR β=1
 *   1M              7.7%       4.3%        0.2%
 *   3M              5.5%       2.4%        0.0%
 *   10M             4.0%       2.7%        0.0%
 *
 * CFR+ decays too slowly to reach zero and β=0 stops decaying at all, because
 * both cap the debt: one at zero, the other at twice a step's worth. β=1
 * discounts it by t/(t+1) instead, which is almost nothing once a solve is
 * running, and the residue goes away.
 *
...
export const DCFR_DEFAULTS = { alpha: 1.5, beta: 1, gamma: 2, every: 50000 };
...
export class Solver {
  constructor(options = {}) {
    ...
    /**
     * How often a sampled seat is made to take a random legal action instead of
     * its own.
     *
     * Without it a line nobody plays is a line nobody learns. The average
     * strategy only accumulates where play goes, so a hand the button folds
     * every time never acquires a strategy for having opened - and then opening
     * is priced as opening and playing randomly, which is a disaster, which
     * keeps it folding. Six-handed at 10M, buckets the button never opened had
     * a millionth of the training behind their post-open decisions that opened
     * buckets had, and three solves disagreed about which jack-high draws to
     * open because whichever fell to zero first stayed there.
     *
     * Exploration is not free: sampling a seat's action from something other
     * than its strategy biases the regrets it feeds, by order `explore`. The
     * honest correction - weighting by the ratio of the two - would give the
     * explored lines a weight of zero, which is the whole thing it is trying to
     * fix, so the bias is taken and kept small instead. The average accumulates
     * the strategy and not the exploration, so what the viewer reads is still
     * the strategy.
     */
  ...
  traverse(nodeId, traverser, hands, reserve, depth) {
    ...
    let picked = -1;
    // `explore > 0` first, so a solve without exploration draws exactly the
    // same numbers in the same order as it always did.
    if (this.explore > 0 && this.rng() < this.explore) {
      let legal = 0;
      for (let a = 0; a < actions; a += 1) if (!mask || mask[a]) legal += 1;
      let at = Math.floor(this.rng() * legal);
      for (let a = 0; a < actions; a += 1) {
        if (mask && !mask[a]) continue;
        if (at === 0) {
          picked = a;
    ...
    // Whatever happened above, act: a masked-out or unspent roll is not a reason
    // to walk off the tree.
    if (picked < 0) {
      for (let a = actions - 1; a >= 0; a -= 1) {
        if (!mask || mask[a]) {
          picked = a;
