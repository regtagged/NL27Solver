...
function placeOf(row) {
  if (row.draw === 0) {
    return { levels: row.ranks.slice(0, 2), leaf: show(row.ranks), full: row.ranks };
  }
  const keep = keepRanksOf(row);
  if (keep.length === 0) return { levels: [], leaf: null, full: [] };
  if (row.draw <= 2) {
    const target = bestCompletion(keep);
    return { levels: target.slice(0, 2), leaf: show(keep), full: target };
  }
  return { levels: keep.slice(0, Math.min(2, keep.length)), leaf: null, full: keep };
}
...
export function groupRows(rows) {
  ...
  for (const row of rows) {
    ...
    const { levels, leaf, full } = placeOf(row);
    let level = category;
    for (let depth = 1; depth <= levels.length; depth += 1) {
      const slice = full.slice(0, depth);
      const key = show(slice);
      let node = level.children.get(key);
      if (!node) {
        node = { label: key, kind: 'group', path: slice, combos: 0, rows: [], children: new Map() };
    ...
    if (leaf) {
      let node = level.children.get(leaf);
      if (!node) {
        // Leaves sort under their own cards, which for a draw is the keep.
        const sortPath = row.draw === 0 ? full : keepRanksOf(row);
        node = { label: leaf, kind: 'hand', path: sortPath, combos: 0, rows: [], children: new Map() };
        level.children.set(leaf, node);
      }
      node.combos += row.combos;
      node.rows.push(row);
    }
