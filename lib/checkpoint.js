...
import { defaultConfig } from './tree.js';
import { DCFR_DEFAULTS } from './solve.js';

/** A name that says which game this is, so the file is self-describing. */
export function solveKey(config, joint, iterations, algorithm = 'cfr+', discount = null, explore = 0) {
  const ante = config.ante ? `-a${config.ante}${config.anteMode}` : '';
  // The same rule for sizing and for the algorithm: what existed when solves
  // were first stored keeps the unmarked name, and anything else says what it
  // is. Two sizings of one game must not overwrite each other's solve.
  const open = config.openTo != null && config.openTo !== defaultConfig().openTo
    ? `-o${config.openTo}` : '';
  const threeBet = config.threeBetTo
    ? `-3b${config.threeBetTo.inPosition}_${config.threeBetTo.blinds}` : '';
  // β decides whether a dominated action stays buried, so two solves that
  // differ by it are different answers and must not be one file.
  const beta = discount?.beta ?? DCFR_DEFAULTS.beta;
  const cushion = algorithm === 'dcfr' && beta !== DCFR_DEFAULTS.beta ? `-b${beta}` : '';
  const method = algorithm === 'cfr+' ? '' : `-${algorithm}${cushion}`;
  // Exploration changes which lines get trained, so it changes the answer.
  const explored = explore ? `-e${explore}` : '';
  return `${config.players}p-${config.stack}bb-${config.smallBlind}_${config.bigBlind}${ante}`
    + `${open}${threeBet}${joint ? '-joint' : '-predraw'}${method}${explored}`
    + `-${Math.round(iterations / 1e6)}M`;
}
...
export function load(solver, file) {
  ...
  if (head.nodes !== solver.nodes.length
    ...
    || (head.algorithm ?? 'cfr+') !== solver.algorithm
    || (head.discount?.beta ?? null) !== (solver.discount?.beta ?? null)
    || (head.explore ?? 0) !== solver.explore
