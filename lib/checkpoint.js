...
export function solveKey(config, joint, iterations, algorithm = 'cfr+') {
  const ante = config.ante ? `-a${config.ante}${config.anteMode}` : '';
  // CFR+ was the only algorithm when solves were first stored, so it keeps the
  // unmarked name and anything else says what it is.
  const method = algorithm === 'cfr+' ? '' : `-${algorithm}`;
  return `${config.players}p-${config.stack}bb-${config.smallBlind}_${config.bigBlind}${ante}`
    + `${joint ? '-joint' : '-predraw'}${method}-${Math.round(iterations / 1e6)}M`;
}
